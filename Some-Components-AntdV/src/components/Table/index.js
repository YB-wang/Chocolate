...
export default {
  ...
  mounted() {
    if (this.$attrs.tableSettingID && localStorage.getItem(this.$attrs.tableSettingID)) {
      this.columnsSetting = JSON.parse(localStorage.getItem(this.$attrs.tableSettingID));
    }
  },
  methods: {
    rowClick(record, index) {
      return {
        on: {
          click: () => {
            if (this.highlightItem && this.highlightItem.id === record.id) {
              //行点击选择-高亮
              this.highlightItem = null;
              this.highlightIndex = NaN;

            } else {
              this.highlightIndex = index;
              this.highlightItem = record;
              
              
            }
            this.$emit('highlight',this.highlightItem)
          }
        }
      }
    },
    setRowClassName(record) {
      if (this.highlightItem && record.id === this.highlightItem.id) {
        return 's-table-row-highlight'

      } else {
        return 's-table-row-no-highlight'
      }
     ...
     * @param Boolean bool
     */
    setlocalDataSource(arr) {
      this.localDataSource = arr;
      this.localDataSource.push({});
      this.localDataSource.pop();
    },
    refresh(bool = false) {
    ...
    columnChange(val) {
      ...
      if (this.$attrs.tableSettingID) {
        localStorage.setItem(this.$attrs.tableSettingID, JSON.stringify(val));
      }
    },
    renderHeader() {
      let tools = [
