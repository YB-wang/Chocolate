...
export default {
  ...
  mounted() {
    this.columnsSetting = this.columns.map(value => value.checked === false ? value : ({ ...value, checked: true }));
    this.originColumns = this.columns.map(value => ({ ...value, checked: true }));
    if(this.columnsSetting.every(value => value.checked!==false)){
      this.checkAll=true;
    }
